...
const italianSidebar = ({ selected, onSelect }) => (
  <aside className="w-full md:w-64 bg-white shadow-lg rounded-lg p-4 mt-4 md:mt-0">
    ...
    <ul className="space-y-4">
      {features.map((title) => (
        <li key={title}>
          <button
            className={`w-full text-left font-semibold text-green-700 px-2 py-2 rounded transition-colors duration-200 focus:outline-none ${
